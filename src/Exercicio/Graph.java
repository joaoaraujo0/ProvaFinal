...
public class Graph {
  ...
  public List<Vertex> buscaEmLargura(String startVertex, String endVertex) {
	    Vertex start = findVertex(startVertex);
	    Vertex end = findVertex(endVertex);
	    ...
	    while (!queue.isEmpty()) {
	        ...
	        for (Edge edge : visited.getEdges()) {
	            Vertex next = edge.getTo();

	            if (!marked.contains(next)) {
	                marked.add(next);
	                parentMap.put(next, visited);
	                queue.add(next);
	            }
	        }
	    }
